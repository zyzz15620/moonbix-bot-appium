...
public class ActionsUtils {
    ...
    public static void tapAtCoordinates(int x, int y) {
        try {
            executeTap(x, y, Duration.ofMillis(100));  // 100ms tap
        } catch (Exception e) {
    ...
    public static void longPressAtCoordinates(int x, int y, Duration duration) {
        try {
            executeTap(x, y, duration);  // Use custom duration for long press
        } catch (Exception e) {
    ...
    public static void doubleTap(int x, int y) {
        try {
            executeTap(x, y, Duration.ofMillis(50));  // First tap
            executeTap(x, y, Duration.ofMillis(50));  // Second tap shortly after
        } catch (Exception e) {
            handleException(e, "double tapping at coordinates");
        }
    }

    // Pinch action for zoom out
    ...
    public static void drag(int startX, int startY, int endX, int endY, Duration duration) {
        try {
            ...
            dragSequence.addAction(new Pause(finger, Duration.ofMillis(200))); // Optional pause before drag
    ...
    // Helper to execute taps with a custom duration
    ...
    private static void multiTouch(int centerX, int centerY, int startX1, int startY1, int startX2, int startY2, Duration duration) {
        ...
        // Perform the multi-touch gesture (pinch or zoom)
